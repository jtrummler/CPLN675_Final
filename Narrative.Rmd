...
Looking at our model with the most variables, we see _____.
...
The McFadden Score, also known as the pseudo R-squared, helps give us insight to explaining how much our variables explain variance in land patterns. Our model _ has the highest R-squared, though all of the models are low.
...
We experiment with two probability thresholds: __ and __. These thresholds help us binarize probabilities, so that anything falling under the threshold gets assigned 0 (no change), and anything above the threshold gets assigned 1 (new development). We chose the values __ and __ because __.

We see that from the table, __.
...
This plot indicates that as we __, we __.
...
A confusion matrix is a way to assess how a determine threshold tests on four metrics:

*True Positive*: Predicted developed, actually developed.  
*True Negative*: Predicted unchanged, actually unchanged.  
*False Positive*: Predicted developed, actually unchanged.  
*False Negative*: Predicted unchanged, actually developed.  

We hope to optimize for true positives and true negatives, as they inherently increase our accuracy. In this plot we see the true positives and negatives for our two selected thresholds. It shows __.
...
From the table below, we see __.
...
We further develop this model to try to predict land cover demand in 2030, connceting back to our use case that planners can use this sort of model to understand the spatial patterns of land use change. We begin this process by analyzing population change between 2019 and 2029 at the county level.
...
We calculate infill pontential of each county and feed it into our model 6 from the modeling section, and look at the spatial characteristics of demand in 2029. We see __.
...
dat_infill %>%
  ...
  labs(title= "Development Demand in 2030", subtitle="Nashville MSA") +
  mapTheme()
```

## Development Demand and Environmental Sustainability

The pressures of demand often must be balanced with the supply of land appropriate for developed regions. Often, unsustainable land use planning has resulted in leap-frog patterns of development and landscape fragmentation, whereas sustainable land use planning would promote densifying already developed land and promoting infill in continuous developed regions. We analyze land use change between our two study years and see how much sensitive land (forest and wetlands) was lost, and aggregate it to the county level.
...
ggplot() +
  ...
  scale_colour_manual(values = palette2,
                      ...
                      name = "") +
  labs(title = "Loss of Sensitive Lands, as fishnet centroids",
       subtitle = "Nashville MSA") +
  mapTheme()
```

Now, we analyze county specific metrics. __
...
county_specific_metrics %>%
  ...
  ggplot(aes(x=Variable, y=Value, fill=Planning_Designation)) +
    ...
    scale_fill_manual(values=c(paletteMain[1], paletteMain[2], paletteMain[3]) +
    labs(title= "Specific Allocation Metrics", subtitle= "Nashville MSA Counties", x="Indicator", y="Rate") +
    plotTheme + theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position="bottom")
```

# Supply-Side Solution: Interstate Expansion

We assess this model for a supply-side solution too. We decided to add a proposed Interstate expansion to the Nashville MSA, as the core area of Davidson County/Nashville has a pretty robust interstate network, but other counties that may be experiencing greater development would see increased growth with interstate accessibility. First, we look at our proposed interstate.

*Insert code for interstate*
...
```{r}
Model7 <- glm(layer ~ wetlands + forest  + farm + otherUndeveloped + lagDevelopment + pop_Change + highway_dist + city_dist + PBNEC_dist, 
